...
class MultiHeadedSelfAttention(nn.Module):
    ...
    def __init__(self, dim, num_heads, dropout, num_particles):
        super().__init__()
        self.proj_q = nn.Linear(dim, dim, num_particles)
        self.proj_k = CustomLinear(dim, dim, num_particles)
        self.proj_v = nn.Linear(dim, dim, num_particles)
...
class PositionWiseFeedForward(nn.Module):
    ...
    def forward(self, x):
        ...
        final_res = []
        for i in range(self.num_particles):
            res = self.fc2(F.gelu(self.fc1(x[i])))
...
class CustomLayerNorm(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(self.num_particles):
            ...
            # print(self.gamma, self.beta, x[i], self.eps)
            res = self.gamma * (x[i] - mean) / (std + self.eps) + self.beta
            final_res.append(res)
        return final_res
...
class CustomLinear(nn.Module):
    def __init__(self, dim_in, dim_out, num_particles):
        ...
        self.weight = nn.Parameter(torch.randn(self.in_features, self.out_features))
    ...
    def forward(self, x):
        final_res = []
        for i in range(self.num_particles):
            res = torch.matmul(x[i], self.weight) + self.bias
            final_res.append(res)
        return final_res
